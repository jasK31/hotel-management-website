...
<div class="container-fluid">
	...
	<form action="" id="manage-check">
		...
		<div class="form-group">
			...
			<input type="text" name="name" id="booking-name" class="form-control" value="<?php echo isset($meta['name']) ? $meta['name'] : '' ?>" required>
		</div>
		<div class="form-group">
	...
	$('#manage-check').submit(function(e) {
		...
		$.ajax({
			...
			success: function(resp) {
				var name1 = document.getElementById('booking-name').value;
				var contact = document.getElementById('contact').value;
				var date_in = document.getElementById('date_in').value;
				var date_in_time = document.getElementById('date_in_time').value;
				var days = document.getElementById('days').value;


				var qs = '';
				qs = qs + '&name=' + name1;
				qs = qs + '&contact=' + contact;
				qs = qs + '&date_in=' + date_in;
				qs = qs + '&date_in_time=' + date_in_time;
				qs = qs + '&days=' + days;
				...
				if (resp > 0) {
					...
					setTimeout(function() {
						end_load()
						$('.modal').modal('hide')
						window.location.replace('http://localhost/p2/jashan-project-hm/index.php?page=success' + qs)
					}, 1500)
				}
			},
